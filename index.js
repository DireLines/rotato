...
var ASM_CONSTS = {
  11536014: () => { if (document.fullscreenElement) return 1; },  
 11536060: () => { return Module.canvas.width; },  
 11536092: () => { return parseInt(Module.canvas.style.width); },  
 11536140: () => { document.exitFullscreen(); },  
 11536167: () => { setTimeout(function(){ Module.requestFullscreen(false, false); }, 100); },  
 11536239: () => { if (document.fullscreenElement) return 1; },  
 11536285: () => { return Module.canvas.width; },  
 11536317: () => { return screen.width; },  
 11536342: () => { document.exitFullscreen(); },  
 11536369: ($0) => { const canvasId = UTF8ToString($0); setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { document.querySelector(canvasId).style.width="unset"; }, 100); }, 100); },  
 11536563: () => { return window.innerWidth; },  
 11536589: () => { return window.innerHeight; },  
 11536616: () => { if (document.fullscreenElement) return 1; },  
 11536662: () => { return Module.canvas.width; },  
 11536694: () => { return parseInt(Module.canvas.style.width); },  
 11536742: () => { if (document.fullscreenElement) return 1; },  
 11536788: () => { return Module.canvas.width; },  
 11536820: () => { return screen.width; },  
 11536845: () => { return window.innerWidth; },  
 11536871: () => { return window.innerHeight; },  
 11536898: () => { if (document.fullscreenElement) return 1; },  
 11536944: () => { return Module.canvas.width; },  
 11536976: () => { return screen.width; },  
 11537001: () => { document.exitFullscreen(); },  
 11537028: () => { if (document.fullscreenElement) return 1; },  
 11537074: () => { return Module.canvas.width; },  
 11537106: () => { return parseInt(Module.canvas.style.width); },  
 11537154: () => { document.exitFullscreen(); },  
 11537181: ($0) => { Module.canvas.style.opacity = $0; },  
 11537219: () => { return screen.width; },  
 11537244: () => { return screen.height; },  
 11537270: () => { return window.screenX; },  
 11537297: () => { return window.screenY; },  
 11537324: () => { return window.devicePixelRatio; },  
 11537360: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 11537413: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 11537464: () => { Module.canvas.style.cursor = 'none'; },  
 11537501: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 11537757: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 11537808: () => { if (document.pointerLockElement) return 1; },  
 11537855: () => { if (document.fullscreenElement) return 1; },  
 11537901: () => { return window.innerWidth; },  
 11537927: () => { return window.innerHeight; },  
 11537954: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; let miniaudio = window.miniaudio; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === miniaudio.device_state.started) { device.webaudio.resume().then(() => { _ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 11540132: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, window.miniaudio.unlock, true); }); window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 11540436: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 11540540: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 11543588: ($0) => { return window.miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 11543661: ($0) => { var device = window.miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 11544061: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 11544111: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 11544250: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; }
};
